...
const lineNumberify = function lineNumberify(ast: any[], lineNum = 1) {
  ...
  return ast.reduce(
    (
      result: {
        nodes: { type: string; value: any; lineNumber: number }[];
        lineNumber: number;
      },
      node: { type: string; value: string; lineNumber: number; children: any }
    ) => {
      if (node.type === "text") {
...
const wrapLines = function wrapLines(
  ast: any[],
  linesToHighlight: string | unknown[]
) {
  const highlightAll =
    linesToHighlight.length === 1 && linesToHighlight[0] === 0;
  const allLines = Array.from(
    new Set(ast.map((x: { lineNumber: any }) => x.lineNumber))
  );
  let i = 0;
...
const applyMultilineFix = function (ast: any) {
  ...
  // Fix JSX issue
  html = html.replace(MULTILINE_TOKEN_SPAN, (match: string, token: any) =>
...
export const exported = (ast: any, lines: any) => {
